...
describe.runIf(databaseUrl)('зачистка просроченных refresh-сессий', () => {
  ...
  it('сохраняет отозванную сессию, просроченную внутри grace-периода (revokedAt не ускоряет удаление)', async () => {
    const userId = await createTestUser();
    const sessionId = await insertSession(userId, daysFromNow(-3), new Date());
  ...
  it('удаляет отозванную сессию, просроченную дольше grace-периода (revokedAt не защищает от удаления)', async () => {
    const userId = await createTestUser();
    const sessionId = await insertSession(userId, daysFromNow(-8), new Date());
